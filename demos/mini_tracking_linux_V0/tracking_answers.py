...
# Function to calculate the vector move distance (shift value) for each marker
def calculate_shift_distance(Ox, Oy, Cx, Cy):
    # Calculate Euclidean distance between original and current coordinates
    shift_distances = np.sqrt((Cx - Ox)**2 + (Cy - Oy)**2)
    return shift_distances

# Function to calculate the torsional displacement for each point
def calculate_torsional_displacement(Ox, Oy, Cx, Cy):
    # Calculate angle of rotation for the original (Ox, Oy) and current (Cx, Cy) positions
    theta_O = np.arctan2(Oy, Ox)  # Original angle
    theta_C = np.arctan2(Cy, Cx)  # Current angle
    
    # Calculate torsional displacement (difference in angle)
    torsional_displacement = np.abs(theta_C - theta_O)
    
    # Normalize the angle differences to the range [0, 2*pi]
    torsional_displacement = np.mod(torsional_displacement, 2 * np.pi)
    
    return torsional_displacement
    ...
    try:
        while (WHILE_COND):
            ...
            if calibrate == False:
                ...
                total_diff = 0
                total_tor_displace = 0
                max_tor_displace = 0
                count = 0
                for i in range(len(Ox)):
                    for j in range(len(Ox[i])):
                        diff = calculate_shift_distance(Ox[i][j], Oy[i][j], Cx[i][j], Cy[i][j])
                        tor_displace = calculate_torsional_displacement(Ox[i][j], Oy[i][j], Cx[i][j], Cy[i][j])
                        total_diff += diff
                        total_tor_displace += tor_displace
                        if tor_displace > max_tor_displace:
                           max_tor_displace = tor_displace
                        count += 1
                        # print(
                        #     f"frame:{frameno}, Ox:{i}, Oy:{j}, {Ox[i][j]:.2f}, {Oy[i][j]:.2f}, {Cx[i][j]:.2f}, {Cy[i][j]:.2f},different:{diff}\n")
                if count != 0:
                    mean_diff = total_diff / count
                    mean_tor_displace = total_tor_displace / count
                else:
                    mean_diff = 0     
                print (f"1.Now the mean_diff is:{mean_diff}\n2.Now the mean_tor_displace is:{mean_tor_displace}\n3.Now the max_tor_displace is:{max_tor_displace}\n") 
